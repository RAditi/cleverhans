...
from cleverhans.model_zoo import modelAllConvolutional
...
"""
  ...
  def evaluate():
    ...
    test_clean_accuracy, test_clean_loss = do_eval(preds, x_test, y_test, 'test_clean_eval', False)
    # Accuracy of the adversarially trained model on adversarial examples
    test_adv_accuracy, test_adv_loss = do_eval(preds_adv, x_test, y_test, 'test_adv_eval', True)
    # Accuracy on train clean 
    train_clean_accuracy, train_clean_loss = do_eval(preds, x_train, y_train, 'train_clean_eval', False)
    # Accuracy on train adv 
    train_adv_accuracy, train_adv_loss = do_eval(preds_adv, x_train, y_train, 'train_adv_eval', True)
...
def main(argv=None):
 ...
 if __name__ == '__main__':
  ...
  flags.DEFINE_float('clean_weight', CLEAN_WEIGHT,
                       'Weight on loss of clean examples')
  flags.DEFINE_float('adv_weight', ADV_WEIGHT,
                       'Weight on loss of adversarial examples')
